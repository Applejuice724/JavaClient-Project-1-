 ...
 */
package client_project.GUI.MainMenu.ModTools;

import client_project.ApplicationStateManager;
import client_project.ApplicationStateManager.ModAccess;
import static client_project.UserInformation.NetControl.bytesToHex;
...
public class modAccess {
    private static ModAccess Action;
    ...
    private static String SQLAddUserMessage;
    private static ActionListener actionlisten  = new ActionListener() 
    {
        ...
        public void actionPerformed(ActionEvent e) {
            if (e.getSource().equals(SubmitUserInformation))
            {
                ...
                {
                    SQLAddUserMessage = 
                            (" (f_name, l_name ,email ,username,password) values( '"
                            +FirstNameInputFeild.getText()+"', '"
                            +lastNameInputFeild.getText()+"', '"
                            +EmailInputFeild.getText()+"',  '"
                            +UsernameInputFeild.getText()+"', '"
                            +GetPassword()+"');");  
                    Action = ModAccess.ADDUSR;                         
                }
                FirstNameInputFeild.setText("");
                lastNameInputFeild.setText("");
                EmailInputFeild.setText("");
                UsernameInputFeild.setText("");
                PasswordInputFeild.setText("");                                
            }            
        }
    };
    public String getAddUserString()
    {
        return SQLAddUserMessage; 
    }
    public boolean checkActionPerformed()
    {
        if(Action == ModAccess.NONE) return false;
        else{return true;}
    }
    public ModAccess getCurrentAction()
    {
        ModAccess temp = Action;
        Action = ModAccess.NONE;
        return temp;
    }
    private static boolean ifValidInputs()
    {
        String username_w = UsernameInputFeild.getText();
        if (username_w.equals(null))return false;
        String Firstname_w = FirstNameInputFeild.getText();
        if (Firstname_w.equals(null))return false;
        String Lastname_w = lastNameInputFeild.getText();                
        if (Lastname_w.equals(null))return false;
        String Email_w = EmailInputFeild.getText();                
        if (Email_w.equals(null))return false;
        String Password_w = GetPassword();                
        if (Password_w.equals("*BE1BDEC0AA74B4DCB079943E70528096CCA985F8"))return false;
        return true;
    }
